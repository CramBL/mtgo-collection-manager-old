...
template <class T>
concept goatbots_json =
    std::disjunction<std::is_same<T, price_hist_map_t>,
                     std::is_same<T, card_defs_map_t>>::value;

template <goatbots_json T>
[[nodiscard]] auto ReadJsonMap(std::filesystem::path path_json)
    -> std::optional<T> {
...
template <> struct glz::meta<goatbots::CardDefinition> {
  ...
  static constexpr auto value = object("name", &T::name, "cardset", &T::cardset,
                                       "rarity", &T::rarity, "foil", &T::foil);
};